...
defaultFilterZeroMutGene = 'no';
validFilterZeroMutGene = {'no','yes'};
checkFilterZeroMutGene = @(x) any(validatestring(x,validFilterZeroMutGene));
...
addOptional(p,'Filter_zero_mut_gene',defaultFilterZeroMutGene,checkFilterZeroMutGene)
...
if strcmp('yes',cellstr(defaultVisualization))
   ...
   % boxplot(gl(y),x,'Symbol','','Notch','on','Colors',cm);hold on;
...
OUTPUT_STRUCTURE.gname = gname(i);
OUTPUT_STRUCTURE.gclu = gclu;
OUTPUT_STRUCTURE.glength = vglength(j);
OUTPUT_STRUCTURE.var = vnum(j);
OUTPUT_STRUCTURE.var_per_kb = variants;
