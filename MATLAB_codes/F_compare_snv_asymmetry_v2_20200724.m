...
%    'Filter_zero_mut_gene': Filter the genes with zero mutation records
%                            or not. default 'yes'.
%                            Optional, 'no'
...
addOptional(p,'Filter_zero_mut_gene',defaultFilterZeroMutGene,checkOption)
...
defaultFilterZeroMutGene = p.Results.Filter_zero_mut_gene;
...
if strcmp('yes',cellstr(defaultFilterZeroMutGene))
    mutated_genes = find(sum(vnum,2)>0);
    vnum    = vnum(mutated_genes,:);
    %vglength = vglength(mutated_genes);
...
if strcmp('single_gene',cellstr(default_Mode))
    ...
    for i = 1:length(asym_order)
        ...
        vnum_sorted(:,strcmp(asym_order(i),asym_order)) = vnum(:,strcmp(asym_order(i),vtype_names));
    end
...
if strcmp('yes',cellstr(default_F_compare_variants))
    ...
    variants_level = F_compare_variants_20200724(sum(vnum,2),variants_genelength,v_genenames,...
                                        gclu,gname,face_cm);
...
%         ci = bootci(10000,{@mean,vgc},'alpha',0.01)-mean(vgc);  %mean of strand_SNV
        if length(vgc)<1
            fprintf('Cluster %d only has %d genes, boostrap sampling skipped.',j,length(vgc))
            continue
        end
...
OUTPUT_STRUCTURE.vtype          = default_snv_asymmetry;
OUTPUT_STRUCTURE.gcluster       = gclu;
OUTPUT_STRUCTURE.vglength       = vglength;
OUTPUT_STRUCTURE.vnum_pergene   = vnum_sorted;
OUTPUT_STRUCTURE.vnum_perkb     = vnum_perkb;
OUTPUT_STRUCTURE.gname        = gname;
OUTPUT_STRUCTURE.base_names     = base_names;
